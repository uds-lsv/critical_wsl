...
class CosineNERTrainer(Trainer):
    def __init__(self, args, logger, log_dir, random_state):
        super(CosineNERTrainer, self).__init__(args, logger, log_dir, random_state)
    ...
    def calc_loss(self, input_log_prob, target, classification_loss, device, thresh=0.95, soft=True, conf='max', confreg=0.1):
    ...
    def train_cosine_student(self, args, logger, init_cosine_model, data_dict):
        ...
        self.logger.info(f"student train_set acc: {np.average(np.array(train_set.n_labels) == np.array(train_set.labels))}")
        ...
        for step in range(num_training_steps):
            ...
            ner_label_mask = (input_labels.view(-1) != -100).to(device)  # labels on which the loss is computed on


            outputs = student_model(input_batch)
            outputs_pseudo = teacher_model(input_batch)

            outputs_logits_flatten = (outputs['logits']).view(-1, self.num_classes)[ner_label_mask]
            outputs_pseudo_flatten = (outputs_pseudo['logits']).view(-1, self.num_classes)[ner_label_mask]

            outputs_pseudo_prob = F.softmax(outputs_pseudo_flatten, dim=1)
            log_soft_logits = torch.log(F.softmax(outputs_logits_flatten, dim=1))
            ...
            if args.self_training_contrastive_weight > 0:
                feat = outputs_pseudo['last_hidden_state']
                feat_flatten =  feat.view(-1, feat.size(-1))
                feat_flatten_final = feat_flatten[ner_label_mask]

                contrastive_loss = self.contrastive_loss(input=log_soft_logits,
                                                         feat=feat_flatten_final,
                                                         target=outputs_pseudo_flatten,
                                                         device=device,
                                                         conf='entropy',
                                                         thresh=args.self_training_eps,
    ...
    def train(self, args, logger, full_dataset):
        ...
        assert args.task_type == "ner", "this cosine NER trainer only supports NER tasks"
        logger.info('COSINE NER Trainer: Training started')
        ...
        if args.small_validation:
            if args.load_from_teacher_small_val:
                ...
                val_set = val_subset
                print(f"Example of selected index: {subset_idx[:10]}...")
