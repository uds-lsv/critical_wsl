...
class LearnToReweightNERTrainer(Trainer):
    def __init__(self, args, logger, log_dir, random_state):
        super(LearnToReweightNERTrainer, self).__init__(args, logger, log_dir, random_state)
    ...
    def train(self, args, logger, full_dataset):
        assert args.task_type == 'ner', "this trainer only supports NER tasks"
        ...
        for step in tqdm(range(num_training_steps), desc=f'training steps', ncols=150):
            ...
            tr_input_labels = tr_input_batch['labels']
            ner_label_mask = (tr_input_labels.view(-1) != -100).to(device)  # labels on which the loss is computed on
            ...
            if len(tr_input_labels.detach().shape) > 1:  # ner task
                eps_length = bs * args.max_sen_len
            else:
                eps_length = bs
            ...
            with higher.innerloop_ctx(meta_net, meta_optimizer) as (fmodel, diffopt):
                ...
                eps = torch.zeros(eps_length, requires_grad=True).to(device)
                l_f_meta = torch.mean((eps * loss_fn_no_reduction(meta_pred, tr_input_batch['labels']))[ner_label_mask])
            ...
            if args.task_type == "ner":
                norm_c = torch.sum(w_tilde[ner_label_mask])
            elif args.task_type == "text_cls":
                norm_c = torch.sum(w_tilde)
            else:
                raise NotImplementedError("[Learning2Reweight]: unknown task")
            ...
            loss = torch.mean((w.detach() * loss_fn_no_reduction(outputs, tr_input_batch['labels']))[ner_label_mask])
            loss.backward()
